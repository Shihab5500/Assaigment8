...
//   const [sort, setSort] = useState("sizeAsc");
...
//     uninstall(app.id);
//     setToast(`${app.title} un-installed from your Device`);
//   };
...
//       <p className="text-center text-gray-500 mt-1">Explore All Trending Apps on the Market developed by us</p>

//       <div className="mt-6 flex items-center justify-between">
//         <div className="text-sm text-gray-700">{installed.length} Apps Found</div>
//         <select
//           value={sort}
//           onChange={(e)=>setSort(e.target.value)}
//           className="px-3 py-2 rounded-md border border-gray-200 text-sm"
//         >
//           <option value="sizeAsc">Sort By Size ↑</option>
...
//                   <span className="px-1.5 py-0.5 rounded bg-emerald-50 text-emerald-600">⬇ {Math.round(app.downloads/1e6)}M</span>
//                   <span className="px-1.5 py-0.5 rounded bg-orange-50 text-orange-600">★ {app.ratingAvg}</span>
//                   <span className="px-1.5 py-0.5 rounded bg-sky-50 text-sky-600">{app.size} MB</span>
//                 </div>
//               </div>
//             </div>
//             <button
...
export default function Installation() {
  const { installed, uninstall } = useInstall();
  const [toast, setToast] = useState("");
  // ✅ Downloads অনুযায়ী sort
  const [sort, setSort] = useState("dlAsc"); // dlAsc | dlDesc
